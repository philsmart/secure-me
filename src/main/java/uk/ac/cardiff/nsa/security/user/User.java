...
public class User {
    ...
    public SimpleToken generateToken() {
        ...
        final SimpleToken token =
                SimpleToken.builder().setPrincipalName(username).setRole(role).setIssuedAt(System.currentTimeMillis())
                        .setValidFor(100000).setNonce(UUID.randomUUID().toString()).build();
